...
public:
    URDFModel(const std::string &urdf_file_name) {
        ...
        dynamics->setWorldGravity({0.0, 0.0, 9.81});
...
//        dynamics->inverseDynamics()
    ...
    void setJointVels(std::vector<double> jointVels) {
        auto curVel = dynamics->getVelocity();

        if (jointVels.size() != curVel.size()) {
            std::stringstream ss;
            ss << "Wrong number of joint. Make sure input is length " << curVel.size() << "\n";
            throw std::runtime_error(ss.str());
        }

        for (int i = 0; i < curVel.size(); i++) {
            curVel[i] = jointVels[i];
        }
        dynamics->setVelocity(curVel);
        dynamics->forwardVelocity();

    }


    void setJointAccels(std::vector<double> jointAccels) {
        auto curAccels = dynamics->getAcceleration();

        if (jointAccels.size() != curAccels.size()) {
            std::stringstream ss;
            ss << "Wrong number of joint. Make sure input is length " << curAccels.size() << "\n";
            throw std::runtime_error(ss.str());
        }

        for (int i = 0; i < curAccels.size(); i++) {
            curAccels[i] = jointAccels[i];
        }
        dynamics->setAcceleration(curAccels);
    ...
    py::array_t<double> getJointsVels() {
        auto curVels = dynamics->getVelocity();

        auto [joints_ptr, capsule] = allocateVector<double>(curVels.size());
        auto &joints = *joints_ptr;
        for (int i = 0; i < curVels.size(); i++) {
            joints[i] = curVels[i];
        }
        auto arr = py::array_t<double>(joints_ptr->size(), joints_ptr->data(), capsule);
        return arr;
    ...
    py::array_t<double> getJointsAccels() {
        auto curAccels = dynamics->getAcceleration();

        auto [joints_ptr, capsule] = allocateVector<double>(curAccels.size());
        auto &joints = *joints_ptr;
        for (int i = 0; i < curAccels.size(); i++) {
            joints[i] = curAccels[i];
        }
        auto arr = py::array_t<double>(joints_ptr->size(), joints_ptr->data(), capsule);
        return arr;
    }

    py::array_t<double> getJointTorques() {
//        dynamics->forwardPosition();
//        dynamics->forwardVelocity();
//        dynamics->forwardAcceleration();
//        dynamics->forwardDynamics();
        dynamics->inverseDynamics();
//        dynamics->inverseForce();
//        dynamics->update();
        auto curTorques = dynamics->getTorque();
        auto [joints_ptr, capsule] = allocateVector<double>(curTorques.size());
        auto &joints = *joints_ptr;
        for (int i = 0; i < curTorques.size(); i++) {
            joints[i] = curTorques[i];
        }
        auto arr = py::array_t<double>(joints_ptr->size(), joints_ptr->data(), capsule);
        return arr;
    }
...
PYBIND11_MODULE(robot_library_py, m) {
    py::class_<URDFModel>(m, "URDFModel")
            ...
            .def("setJointVels", &URDFModel::setJointVels)
            .def("getJointVels", &URDFModel::getJointsVels)
            .def("setJointAccels", &URDFModel::setJointAccels)
            .def("getJointAccels", &URDFModel::getJointsAccels)
            .def("getJointTorques", &URDFModel::getJointTorques)
